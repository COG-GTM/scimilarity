...
class ExpertFFN(nn.Module):
    ...
    d_model: int
        The model dimension.
    d_ff: int
        The feed-forward hidden dimension.
...
class MoELayer(nn.Module):
    ...
    d_model: int
        The model dimension.
    d_ff: int
        The feed-forward hidden dimension per expert.
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Compute weighted combination of selected expert outputs
        ...
        for k in range(self.top_k):
            for e in range(self.num_experts):
                tokens_per_expert[e] += (top_k_indices[:, k] == e).float().sum()
        f = tokens_per_expert / (x_flat.shape[0] * self.top_k)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        -------
        Tuple[torch.Tensor, torch.Tensor]
            (output, moe_aux_loss) where output has the same shape as x.
        """
...
class TransformerMoEEncoder(nn.Module):
    """Transformer+MoE encoder that replaces the MLP Encoder for new models.

    Parameters
    ----------
    n_genes: int
        The number of genes in the gene space.
    latent_dim: int, default: 128
        The latent space dimensions.
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ----------
        x: torch.Tensor
            Input tensor of shape (batch, n_genes).

        Returns
        -------
        Tuple[torch.Tensor, torch.Tensor]
            (embedding, total_aux_loss) where embedding has shape (batch, latent_dim).
        """
        # Pad to make divisible by patch_size
        if self.pad_size > 0:
            x = F.pad(x, (0, self.pad_size))

        # Reshape to patches: (batch, num_patches, patch_size)
...
class TransformerMoEDecoder(nn.Module):
    """Transformer+MoE decoder that replaces the MLP Decoder for new models.
    ...
    def __init__(
        ...
        self.expansion = nn.Linear(latent_dim, self.num_patches * d_model)
        self.pos_embedding = nn.Parameter(
            torch.randn(1, self.num_patches, d_model) * 0.02
        )
        ...
        self.final_norm = nn.LayerNorm(d_model)
        self.output_projection = nn.Linear(d_model, patch_size)
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        -------
        Tuple[torch.Tensor, torch.Tensor]
            (reconstruction, total_aux_loss) where reconstruction has shape (batch, n_genes).
        """
        batch = z.shape[0]
        d_model = self.expansion.out_features // self.num_patches

        # Expand latent to sequence
        x = self.expansion(z)  # (batch, num_patches * d_model)
        x = x.view(batch, self.num_patches, d_model)
        x = x + self.pos_embedding
        ...
        x = self.final_norm(x)
        x = self.output_projection(x)  # (batch, num_patches, patch_size)

        # Reshape and trim
        x = x.reshape(batch, self.num_patches * self.patch_size)
        x = x[:, :self.n_genes]  # trim padding
