...
class TestTransformerBlock:
    def test_output_shape(self):
        block = TransformerBlock(
            d_model=64, n_heads=4, d_ff=128, num_experts=4, top_k=2
        )
        x = torch.randn(BATCH_SIZE, 10, 64)
...
class TestTransformerMoEDecoder:
    ...
    def test_save_load_state(self, tmp_path):
        ...
        for p1, p2 in zip(dec.parameters(), dec2.parameters()):
            assert torch.equal(p1, p2)


class TestExistingModels:
    """Ensure original Encoder/Decoder still work (backward compat)."""
